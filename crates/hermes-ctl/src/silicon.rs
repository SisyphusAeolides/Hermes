...
fn firmware_version_string(version: u64) -> String {
    format!(
        "{}.{}.{:02}",
        version >> 32,
        (version >> 16) & 0xffff,
...
/// Return the source-pinned OpenRM paths accepted for one GPU family, newest
/// release first.  Keeping this list beside the allow-list prevents the host
/// probe from silently falling back to an old firmware directory when a newer
/// admitted image is installed.
pub fn firmware_candidates(root: &Path, family: FirmwareFamily) -> Vec<PathBuf> {
    let mut candidates = Vec::new();
    for manifest in NVIDIA_GSP_RM_DEFAULT_ALLOW_LIST.iter().rev() {
        if manifest.family != family {
            continue;
        }
        let path = root.join(openrm_gsp_relative(
            &firmware_version_string(manifest.version),
            family,
        ));
    ...
    if family == FirmwareFamily::Tu10x {
        candidates.push(root.join(chip_gsp_relative(NvidiaChipDir::Tu117, "570.144")));
        candidates.push(root.join("nvidia/tu117/gsp/gsp-570.144.bin"));
    }
    candidates
}

/// Find the first readable, source-admitted OpenRM image for a device.
/// Reading and authenticating the bytes here keeps callers from selecting a
/// path merely because its filename looks like a supported release.
pub fn find_admitted_firmware(root: &Path, device_id: u16) -> Option<PathBuf> {
    let family = firmware_family_for_device(device_id)?;
    firmware_candidates(root, family)
        .into_iter()
        .find(|path| path.is_file() && probe_firmware_file(path, device_id).admitted)
}
...
pub fn probe_host(firmware_root: &Path) -> SiliconReport {
    ...
    for gpu in &gpus {
        ...
        for c in firmware_candidates(firmware_root, family) {
            if c.exists() {
    ...
    if firmware.is_empty() {
        for (fam, dev) in [
            ...
            for path in firmware_candidates(firmware_root, fam) {
                if path.exists() {
                    let probe = probe_firmware_file(&path, dev);
                    if probe.admitted {
                        any_fw_admit = true;
                    }
                    firmware.push(probe);
...
mod tests {
    ...
    #[test]
    fn firmware_candidates_use_source_pinned_path() {
        let candidates = firmware_candidates(Path::new("/lib/firmware"), FirmwareFamily::Tu10x);
        assert!(candidates
            .iter()
            .any(|path| { path == Path::new("/lib/firmware/nvidia/610.57.04/gsp_tu10x.bin") }));
    }
}
