...
fn load_kmod_script() -> PathBuf {
    let mut candidates = Vec::new();
    if let Ok(root) = std::env::var("HERMES_SCRIPT_ROOT") {
        candidates.push(PathBuf::from(root).join("load-kmod.sh"));
    }
    candidates.push(PathBuf::from("/usr/lib/hermes/scripts/load-kmod.sh"));
    if let Ok(executable) = std::env::current_exe() {
        if let Some(root) = executable
            ...
            .and_then(|p| p.parent())
        {
            candidates.push(root.join("scripts/load-kmod.sh"));
        }
    }
    candidates.push(PathBuf::from("scripts/load-kmod.sh"));
    candidates
        .into_iter()
        .find(|path| path.is_file())
        .unwrap_or_else(|| PathBuf::from("scripts/load-kmod.sh"))
}
