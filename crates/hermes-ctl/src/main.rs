...
fn main() {
    ...
    match args.next().as_deref() {
        ...
        Some("graphics-status") => {
            let report_path = graphics_report_path(&mut args);
            std::process::exit(graphics::status(report_path.as_deref()));
        }
        ...
        Some("silicon-fw-smoke") => {
            std::process::exit(chardev::silicon_fw_smoke());
        }
        _ => {
...
fn graphics_report_path(args: &mut impl Iterator<Item = String>) -> Option<std::path::PathBuf> {
...
fn silicon_bringup_cmd(mode: &str) {
    match mode {
        ...
        "live-fw" => {
            // Load the newest source-admitted host GSP-RM image, then run the
            // complete staging/mailbox path on the simulator. The simulator
            // proves the firmware contract without claiming live GPU Online.
            let path = match silicon::find_admitted_firmware(
                std::path::Path::new("/lib/firmware"),
                0x1fb9,
            ) {
                Some(path) => path,
                None => {
                    println!("PASS (skipped — admitted firmware absent)");
                    return;
                }
            };
            let bytes = match std::fs::read(&path) {
                Ok(b) => b,
                Err(e) => {
                    eprintln!("skip live-fw: cannot read {}: {e}", path.display());
                    println!("PASS (skipped — firmware unreadable)");
                    return;
                }
            };
