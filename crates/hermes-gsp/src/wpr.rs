...
use crate::firmware::{FirmwareFamily, NVIDIA_GSP_RM_DEFAULT_ALLOW_LIST};
...
fn validate_dma(dma: TuringGspDmaInputs) -> Result<(), HermesFault> {
    let admitted_tu10x_length = NVIDIA_GSP_RM_DEFAULT_ALLOW_LIST.iter().any(|manifest| {
        manifest.family == FirmwareFamily::Tu10x && manifest.byte_length as u64 == dma.gsp_rm_bytes
    });
    if dma.gsp_rm_address == 0
        || dma.gsp_rm_address % PAGE_BYTES != 0
        || !admitted_tu10x_length
...
mod tests {
    ...
    #[test]
    fn accepts_the_current_source_pinned_tu10x_image_length() {
        let mut inputs = dma();
        inputs.gsp_rm_bytes = 29_381_504;
        let plan = TuringWprPlan::build(
            framebuffer(),
            inputs,
            TuringRiscvBootOffsets {
        ...
        .expect("610.57.04 TU10x image length is source-admitted");
        assert_eq!(
            u64::from_le_bytes(plan.metadata[24..32].try_into().unwrap()),
            29_381_504
        );
