//! Phase manifold mirroring formal/idris2/HermesAuthority.idr and Agda feature rules.
//!
//! Online is never a free jump from Probe. Callers publish evidence; this
//! module advances only when every gate for the requested transition holds.
...
/// Feature lattice bits aligned with hermes-abi and formal/agda/HermesWire.agda.
pub mod feature {
    ...
    /// Agda HermesWire well-formedness rules.
...
/// Evidence required before Online may be published. Mirrors Idris RawHermesEvidence.
...
mod tests {
    ...
    fn full_gate_chain_reaches_online_with_certificate() {
        ...
        assert!(manifold.negotiate(full_features()).is_ok());
    ...
    fn cannot_jump_to_online_without_gates() {
        ...
        assert!(manifold.negotiate(feature::BOOT_RPC).is_ok());
        // missing wpr/mailbox/ready
        assert_eq!(
            manifold.ignite(false, true, true),
            Err(ManifoldFault::MissingWpr)
        );
    }

    #[test]
    fn incomplete_evidence_never_online() {
        let mut m = HermesManifold::dark(2);
        assert!(m.observe_probe(true).is_ok());
        assert!(m.observe_firmware(true).is_ok());
        assert_eq!(m.arm_queues(false, 1), Err(ManifoldFault::MissingIommu));
        assert_eq!(m.arm_queues(true, 0), Err(ManifoldFault::MissingDomain));
        assert!(m.arm_queues(true, 9).is_ok());
        assert_eq!(m.negotiate(0), Err(ManifoldFault::EmptyFeatures));
        assert_eq!(
            m.negotiate(feature::BOOT_RPC | feature::DISPLAY),
