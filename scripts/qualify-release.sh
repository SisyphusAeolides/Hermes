...
# success and failure.  Full vendor qualification remains strict; the
# host-graphics profile qualifies only the real DRM/Mesa path and records
# vendor-specific tests that are unavailable on the machine.
set -uo pipefail
...
HARDWARE_SCOPE=${HERMES_HARDWARE_SCOPE:-full}
...
case "$HARDWARE_SCOPE" in
    full|host-graphics) ;;
    *)
        printf 'unsupported HERMES_HARDWARE_SCOPE=%s (use full or host-graphics)\n' \
            "$HARDWARE_SCOPE" >&2
        exit 2
        ;;
esac
...
if [[ "$HARDWARE_SCOPE" == host-graphics && -z "$hardware_report" ]]; then
    hardware_report=$OUT/host-graphics-evidence.txt
    graphics_log=$LOG_DIR/host-graphics.log
    if cargo run --frozen --locked --release -p hermes-ctl -- \
        graphics-status --report "$hardware_report" >"$graphics_log" 2>&1; then
        result[host_graphics]=pass
    else
        result[host_graphics]=fail
        record_fail
    fi
fi
...
if [[ -z $hardware_report || ! -r $hardware_report ]]; then
    printf '%s\n' "no hardware evidence report was supplied for scope=$HARDWARE_SCOPE" >"$hardware_log"
...
else
    {
        printf 'report=%s\n' "$hardware_report"
        printf 'scope=%s\n' "$HARDWARE_SCOPE"
        printf 'report_scope=%s\n' \
            "$(sed -n 's/^hardware_scope=//p' "$hardware_report" | head -n 1)"
        printf 'attestation=%s\n' "$(sed -n 's/^attestation=//p' "$hardware_report" | head -n 1)"
        printf 'simulation=%s\n' "$(sed -n 's/^simulation=//p' "$hardware_report" | head -n 1)"
    } >"$hardware_log"
    hardware_ok=1
    report_schema=$(sed -n 's/^schema=//p' "$hardware_report" | head -n 1)
    if [[ "$HARDWARE_SCOPE" == host-graphics ]]; then
        [[ $report_schema == hermes-hardware-host-v1 ]] || hardware_ok=0
        [[ $(sed -n 's/^hardware_scope=//p' "$hardware_report" | head -n 1) == host-graphics ]] || hardware_ok=0
        for gate in graphics_host drm_kms mesa; do
            value=$(sed -n "s/^$gate=//p" "$hardware_report" | head -n 1)
            printf '%s=%s\n' "$gate" "$value" >>"$hardware_log"
            [[ $value == pass ]] || hardware_ok=0
        done
        unavailable=$(sed -n 's/^hardware_unavailable=//p' "$hardware_report" | head -n 1)
        printf 'hardware_unavailable=%s\n' "$unavailable" >>"$hardware_log"
        [[ -n $unavailable ]] || hardware_ok=0
    else
        [[ $report_schema == hermes-hardware-v1 ]] || hardware_ok=0
    fi
    ...
    for gate in "${required_hardware_gates[@]}"; do
        ...
        if [[ "$HARDWARE_SCOPE" == host-graphics ]]; then
            case "$value" in
                pass|not-tested|not-applicable) ;;
                *) hardware_ok=0 ;;
            esac
        else
            [[ $value == pass ]] || hardware_ok=0
...
report_tmp=$TMP_DIR/release-manifest.txt
{
    ...
    if [[ -n $hardware_report && -r $hardware_report ]]; then
        for gate in graphics_host drm_kms mesa hardware_unavailable; do
            value=$(sed -n "s/^$gate=//p" "$hardware_report" | head -n 1)
            [[ -n $value ]] && printf '%s=%s\n' "$gate" "$value"
        done
    fi
...
if [[ -n ${result[host_graphics]:-} ]]; then
    printf '  %-24s %s\n' host_graphics "${result[host_graphics]}"
fi
